...
class TierValidation(models.AbstractModel):
    ...
    def _search_can_review(self, operator, value):
        domain = (
            Domain("review_ids.reviewer_ids", "=", self.env.user.id)
            & Domain("review_ids.status", "in", ["pending", "waiting"])
            & Domain("review_ids.can_review", "=", True)
    ...
    def _search_reviewer_ids(self, operator, value):
        ...
        return Domain("id", model_operator, res_ids)
    ...
    def _get_exception_fields(self, extra_domain=None):
        ...
        domain = (
            ...
            & (
                Domain("group_ids", "in", self.env.user.group_ids.ids)
    ...
    def _notify_review_requested(self, tier_reviews):
        ...
        subscribe = "message_subscribe"
        post = "message_post"
        if tier_reviews.filtered("definition_id.notify_on_create"):
            # Keep the pre-v19 behavior where creation notifications see the
            # first available review as pending, while preserving sequence order.
            tier_reviews._update_review_status()
